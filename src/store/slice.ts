...
const rebaseIds = (tasks: Task[], offset: number): Task[] =>
  tasks.map((t, i) => ({ ...t, id: offset + i }));

const rawNotToDo = localStorage.getItem("Not To Do");
const rawTempted = localStorage.getItem("Tempted");
const rawDidItAnyway = localStorage.getItem("Did It Anyway");

const notToDoBase = rawNotToDo ? JSON.parse(rawNotToDo) : [];
const temptedBase = rawTempted ? JSON.parse(rawTempted) : [];
const didItBase = rawDidItAnyway ? JSON.parse(rawDidItAnyway) : [];

const initialState: TaskState = {
  notToDoTasks: rebaseIds(notToDoBase, 1),
  temptedTasks: rebaseIds(temptedBase, 1000),
  didItAnywayTasks: rebaseIds(didItBase, 2000),
};
...
const taskSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTaskFromNotToDo: (state, action: PayloadAction<number>) => {
      state.notToDoTasks = state.notToDoTasks.filter(
        (task) => task.id !== action.payload,
      );
    },
    deleteTaskFromTempted: (state, action: PayloadAction<number>) => {
      state.temptedTasks = state.temptedTasks.filter(
        (task) => task.id !== action.payload,
      );
    },
    deleteTaskFromDidItAnyway: (state, action: PayloadAction<number>) => {
      state.didItAnywayTasks = state.didItAnywayTasks.filter(
        (task) => task.id !== action.payload,
      );
    },
    ...
    editTaskInNotToDo: (state, action: PayloadAction<Task>) => {
      const index = state.notToDoTasks.findIndex(
    ...
    editTaskInTempted: (state, action: PayloadAction<Task>) => {
      const index = state.temptedTasks.findIndex(
        (task) => task.id === action.payload.id,
      );
      if (index !== -1) {
    ...
    editTaskInDidItAnyway: (state, action: PayloadAction<Task>) => {
      const index = state.didItAnywayTasks.findIndex(
        (task) => task.id === action.payload.id,
      );
      if (index !== -1) {
